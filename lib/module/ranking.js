...
rankingModule.prototype={
     ...
     * @param  {String} userId User ID
     * @param  {Number} top    [Optional]Top result,>0 desc,<0 asc
     * @return {Object}        Promise object
     */
    getDimensionCache:function(userId,top){
        let error;
        top = parseInt(top);
        if(!userId){
        ...
        return redisClient.exists(redisTestKey).then(result=>{ //Confirm redis key
            ...
            this.config.dimensionWeight.forEach(dimensionType=>{
                ...
                if(!top){
                    ...
                    redisPipeline.zrevrange(redisKey,0,top-1,'WITHSCORES');
                }else{
                    redisPipeline.zrange(redisKey,0,-(top+1),'WITHSCORES');
                }
            });
            return redisPipeline.exec();
        }).then(cacheResult=>{
