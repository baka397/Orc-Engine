...
rankingModule.prototype={
    ...
    getDimensionCache:function(userId,dimensionType){
        ...
        return redisClient.exists(redisKey).then(result=>{
            //if not exists this key
            if(parseInt(result)===0){
                return this.setDimensionCache(userId).then(setResult=>{
                    return redisClient.zrevrange(redisKey,0,-1,'WITHSCORES').then(cacheResult=>{
