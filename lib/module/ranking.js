...
function nextPromise(defaultValue){
    return new Promise((resolve,reject)=>{
        if(defaultValue===0) resolve(defaultValue);
...
function redisPointCacheMiddleware(datas,next){
    let addInfo = datas.slice(1);
    let weight = datas[1]*0.5 > datas[2] ? 1 : 2; //Plus gaussian result, for small result group
    let result = Math.round(datas[0]*weight*10000)/10000;
    addInfo.unshift(result)
    next(null,addInfo);
}
...
rankingModule.prototype={
    ...
    setItemPoint(userId,itemId){
        ...
        return this.getItemRecommendCache(userId,1).then(result=>{//Get list count
            ...
            if(itemCount===0) return nextPromise(0);
            ...
            return Promise.all([redisClient.zscore(redisitemListKey,itemId),this.getItemRecommendCache(userId,topNumber)]).then(result=>{//Get avg item,cur item
                ...
                let sigma = 0;
                let totalPoint = 0;
                if(dist===0) return nextPromise([0]);
                result[1].forEach(itemPoint=>{
                    totalPoint+=itemPoint.point;
                })
                sigma = Math.round(totalPoint/result[1].length*10000)/10000;
                return this.getMiddlewares('rankingItemPoint',[gaussianFunc(dist,sigma),itemCount,topNumber]);
            }).then(result=>{//Save key cache
                let point = result[0];
                let redisPipeline = redisClient.pipeline();
                let redisKey = this.getRedisKey(userId+':itemPoint:'+itemId);
                return redisPipeline.set(redisKey,point).expire(redisKey,EXPIRE_TIME).exec().then(resultCache=>{
                    return nextPromise(point);
                });
            })
        })
    },
    getItemPoint(userId,itemId){
        ...
        return redisClient.get(redisKey).then(result=>{ //Confirm redis key
            if(result===null){ //Set cache
                ...
                return nextPromise(parseFloat(result));
            }
        })
    }
}
